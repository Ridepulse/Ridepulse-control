...
# spotify offset steeds -1 doen: eerste nummer =0

PLAYLIST_FILE = 'video_playlists.json'
GOAL_SOUND = 'goal.mp3'
PROLEAGUE_SOUND = 'proleague.mp3'
...
class ScoreboardDisplay(QWidget):
    def __init__(self):
        ...
        # Voeg dit toe in __init__() van je main class (bv. ControlPanel of ScoreboardDisplay)
        # Laad Rubik fonts
...
class ControlPanel(QWidget):
    ...
    def __init__(self, display_window):
        ...
        self.lineup_inputs = []  # For lineup inputs
        self.lineup_index = 0
        self.lineup_files = []
        ...
        # Nieuw: video-loopspeler voor 3e scherm
        self.loop_instance = vlc.Instance()
        self.loop_player = self.loop_instance.media_player_new()
        self.loop_video_path = "Main.mp4"  # standaard video

        # Start loop video op derde scherm
    ...
    def start_loop_video(self):
        if not os.path.exists(self.loop_video_path):
            print(f"Loopvideo niet gevonden: {self.loop_video_path}")
            return
        ...
        # Zet op derde scherm indien beschikbaar
        screens = QApplication.screens()
        if len(screens) >= 3:
            screen = screens[2]  # derde scherm (index 2)
            self.loop_window = QWidget()
            self.loop_window.setWindowFlags(Qt.FramelessWindowHint)
            self.loop_window.showFullScreen()

            # VLC output koppelen aan widget
            if sys.platform.startswith("linux"):
                self.loop_player.set_xwindow(int(self.loop_window.winId()))
            else:
                self.loop_player.set_hwnd(int(self.loop_window.winId()))
        else:
            QMessageBox.warning(self, "Screens issue", "Geen derde scherm gevonden. Led boarding wordt afgespeeld zonder vensterbinding.")


        # Check wanneer video stopt en herstart deze
        def check_loop():
            if self.loop_player.get_state() == vlc.State.Ended:
                self.start_loop_video()
            else:
                QTimer.singleShot(500, check_loop)
    ...
    def set_loop_video(self, video_path):
        if os.path.exists(video_path):
            ...
            QMessageBox.warning(self, "Bestand niet gevonden", f"Kan {video_path} niet vinden")
    ...
    def play_next_video(self):
        ...
        # Ensure the video widget is shown
        ...
        # Wait for valid media length (using polling)
        def check_duration():
    ...
    def show_next_sponsor(self):
        ...
        if ext in [".png", ".jpg", ".jpeg"]:
            ...
            # Nieuwe: actief pollen tot video klaar is
            def poll_video_finished():
                if not self.media_player.is_playing():
                    QTimer.singleShot(100, self.show_next_sponsor)
                else:
                    QTimer.singleShot(500, poll_video_finished)  # check elke halve seconde
    ...
    def play_next_lineup_video(self):
        ...
        # Try to detect when the video ends
